...
fun AppTheme(
    ...
    var barColors = md_theme_light_onPrimaryContainer


    when {
        useDynamicColor && useDarkTheme -> {
            colors = dynamicDarkColorScheme(LocalContext.current)
            barColors = colors.onPrimaryContainer
        }

        useDynamicColor && !useDarkTheme -> {
            colors = dynamicLightColorScheme(LocalContext.current)
            barColors = colors.onPrimaryContainer
        }

        useDarkTheme -> {
            colors = DarkColors
            barColors = md_theme_dark_onPrimaryContainer
        }
    }
